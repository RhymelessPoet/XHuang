...
#include "Render/RenderAPI/RenderContext.h"
...
VulkanRHI::VulkanRHI()
{
    ...
    mDevice = MakeHandle<VulkanDevice>();
    mDebugger = MakeHandle<VulkanDebugger>();
    mSurface = MakeHandle<VulkanSurface>();
}
...
void VulkanRHI::Initialize(const RHI_InitInfo& rhiInfo)
{
    ...
    mCommandPool = MakeHandle<VulkanCommandPool>(mDevice, EQueueType::Graphics);
}
...
void VulkanRHI::GetRequiredExtensions(Vector<const char *>& requiredExtensions)
{
    ...
    requiredExtensions.reserve(glfwExtensionCount + 1u);
...
void VulkanRHI::InitializeDevice()
{
    ...
    if (physicalDeviceCount > 0) {
        ...
        if (mDevice->PickPhysicalDevice(physicalDevices, mSurface)) {
            ...
            VulkanDevice::CreateGraphicsQueue(mDevice);
            VulkanDevice::CreateComputeQueue(mDevice);
            VulkanDevice::CreatePresentQueue(mDevice);
        } else {
...
void VulkanRHI::InitializeSwapchain()
{
    ...
    if (mDevice->GetSwapchainSupportDetails(details)) {
        mSwapchain = MakeHandle<VulkanSwapchain>(mDevice, mSurface, details);
    }
}

IRenderContextHandle VulkanRHI::MakeContext()
{
    VulkanCommandBufferHandle commandBuffer = MakeHandle<VulkanCommandBuffer>(mCommandPool);
    auto renderContext = MakeHandle<VulkanContext>(mDevice, mSwapchain, commandBuffer);
    return MakeHandle<RenderContext<VulkanContext>>(renderContext);
}
